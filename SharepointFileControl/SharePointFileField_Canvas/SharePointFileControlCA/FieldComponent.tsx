...
export class FieldComponent extends React.Component<IFieldComponentProps> {
  ...
  private init = async (props: IFieldComponentProps): Promise<void> => {
    setupUploadFolders(this);
    this.setUpEnability(this, props);
    this.setUpFileName(this, props);
    setUpService(this);
    setUpFileType(this);

    const token = props.context.parameters.token.raw ?? "";
    await this.spService.ensureUploadFolder(token);

    function setupUploadFolders(self: FieldComponent) {
      let path: string;
      let folders: string[];
      const uploadFolderRaw = props.context.parameters.uploadFolder.raw;
      if (!uploadFolderRaw || uploadFolderRaw == '') {
        const entityFolderName = (props.context.mode as any)?.contextInfo.entityTypeName ?? "common";
        const idFolderName = (props.context as any)?.page.entityId ?? "common";
        path = `${entityFolderName}/${idFolderName}`;
        folders = [entityFolderName, idFolderName];
      } else {
        path = uploadFolderRaw;
        folders = uploadFolderRaw.split('/');
      }
      self.uploadFolderPath = path;
      self.uploadFolders = folders;
    }
    function setUpService(self: FieldComponent) {
      ...
      const token = self.props.context.parameters.token.raw ?? "";

      let param: spbuild = {
        appWebUrl: self.appWebUrl,
        library: self.library,
        uploadFolders: self.uploadFolders,
        WebAPI: webApi,
        token: token,
      }
  ...
  setUpFileName(self: FieldComponent, props: IFieldComponentProps): void {
    ...
    function getFullPath() {
      return props.context.parameters.fullPath.raw ?? "";
    }
  ...
  onFileDropped = async (acceptedFiles: any, fileRejections: any, event: any): Promise<void> => {
    ...
    const token = this.props.context.parameters.token.raw ?? "";
    ...
    await this.spService.uploadFile(fileData, token);
    ...
    async function ensureSharePointStatus(self: FieldComponent) {
      const token = self.props.context.parameters.token.raw ?? "";
      await self.spService.ensureUploadFolder(token);
    }
    async function getFileData(self: FieldComponent, fileData: FileData, acceptedFiles: any) {
      ...
      dpFileName = await self.spService.getDuplicateFileName(file.name, token);
      fileData.file = file;
  ...
  generateFullPath = (): void => {
    let parent = `/sites/${this.site}/${this.library}/${this.uploadFolderPath}`;
  ...
  removeFile = async (): Promise<void> => {
    const token = this.props.context.parameters.token.raw ?? "";
    let path = `/sites/${this.site}/${this.library}/${this.uploadFolderPath}`;
    let success = await this.spService.removeFile(path, this.fileName, token);
    if (!success) {
...
function SharePointLink(props: ISharePointLinkProps) {
  ...
  let getFileBlob = async (): Promise<void> => {
    const token = props.component.props.context.parameters.token.raw ?? "";
    const path = constructPath(props.component.fullPath);
    const blob = await props.component.spService.getFileBlob(path, props.component.fileName, token);
    if (blob instanceof Blob) {
      if (blob.size > 0) {
        //const toOnlineView: boolean = fileInfo.mimeType != undefined;
  ...
  let previewFile = async (): Promise<void> => {
    const token = props.component.props.context.parameters.token.raw ?? "";
    const path = constructPath(props.component.fullPath);
    let blob = await props.component.spService.getFileBlob(path, props.component.fileName, token);
    if (blob instanceof Blob) {
      if (blob.size < 1) {
        const alertString = {
          confirmButtonLabel: "好的",
